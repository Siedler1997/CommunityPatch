createPlayer8 = function()
	...
	--StartSimpleJob("ControlP8Serfs")
	StartCountdown(60 * 3, GiveVCtoPlayer5, false)
end

--[[
function ControlP8Serfs()
	local p8SerfAmmount = Logic.GetNumberOfEntitiesOfTypeOfPlayer(2,Entities.PU_Serf)
	...
	if IsExisting("p2_hq") then
		...
		if serfsAtLeftFortress < 4 and p8SerfAmmount < 20 then
	...
	if IsExisting("KI3_HQ") then
		...
		if serfsAtRightFortress < 4 and p8SerfAmmount < 20 then
	...
	if IsExisting("KI2_Outpost") then
		...
		if serfsAtCenterOutpost < 4 and p8SerfAmmount < 20 then
