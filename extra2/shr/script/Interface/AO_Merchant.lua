...
function GUIAction_MerchantCallback(_buidlingId, _heroID)
...
function GUIUpdate_TroopOffer(_SlotIndex)
    ...
    if Logic.IsTechTraderBuilding(SelectedTroopMerchantID) == true then 
        local techType, amount = Logic.GetTechOffer(SelectedTroopMerchantID, _SlotIndex, InterfaceGlobals.CostTable); 
		Amount = amount
        ...
        SourceButton = "Research" .. string.sub(techTypeName, position); 
	else
...
function GUIAction_BuyMerchantOffer(_index)
    ...
    if Logic.IsTechTraderBuilding(SelectedTroopMerchantID) == true then 
        Logic.GetTechOffer(SelectedTroopMerchantID, _index, InterfaceGlobals.CostTable); 
	else
...
function GUITooltip_TroopOffer(_SlotIndex)
    ...
    if Logic.IsTechTraderBuilding(SelectedTroopMerchantID) == true then 
		...
		if XGUIEng.GetStringTableText(nameString) ~= nil then
			TooltipText = TooltipText .. " " .. XGUIEng.GetStringTableText(nameString) .. " @cr " .. XGUIEng.GetStringTableText("MenuMerchant/TechOfferTooltipText");
		else
			TooltipText = TooltipText .. " ??? @cr "..XGUIEng.GetStringTableText("MenuMerchant/TechOfferTooltipText")..""
		end
	else
		local LeaderType, Amount = Logic.GetMercenaryOffer(SelectedTroopMerchantID,_SlotIndex, InterfaceGlobals.CostTable)
		CostString = InterfaceTool_CreateCostString( InterfaceGlobals.CostTable )
